  ...
  bool is_service)
{
  ...
  auto buffer = cdr::SerializationBuffer<false>(nullptr, 0);
  auto serializer = MessageSerializer<false, MessageMembersT>(buffer);
  ...
  const size_t size)
{
  ...
  try {
    auto buffer = cdr::SerializationBuffer<true>(dds_message, size);
